...
func PatchMeshTilePointers(tile *NavMeshTile, data []byte, dataSize int32) bool {
	...
	polyRegionsSize := Align4(unsafe.Sizeof(NavMeshPolyRegion(0)) * uintptr(header.polyCount))
	...
	var polyRegions []NavMeshPolyRegion
	sliceHeader = (*reflect.SliceHeader)(unsafe.Pointer(&(polyRegions)))
	sliceHeader.Cap = int(header.polyCount)
	sliceHeader.Len = int(header.polyCount)
	sliceHeader.Data = 0x00
	if sliceHeader.Len > 0 {
